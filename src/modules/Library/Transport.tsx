...
const Transport: React.FC = () => {
    ...
    // Redux selectors
    const { transports: reduxTransports, ui } = useSelector(
    ...
    const [visibleColumns, setVisibleColumns] = useState<Set<string>>(
        new Set(['vehicleType', 'vehicleNumber', 'capacity', 'price', 'routes', 'vendorName', 'contact', 'rating', 'lastUpdated', 'actions'])
    )
    ...
    const vehicles: VehicleData[] = useMemo(() => {
        return reduxTransports.map((transport) => ({
            id: transport.id,
            ...
            lastUpdated: transport.lastUpdated || new Date().toLocaleDateString(),
        }))
    }, [reduxTransports])
    ...
    const handleDelete = (vehicle: VehicleData) => {
        setDialogConfig({
            ...
            onConfirm: async () => {
                try {
                    await dispatch(deleteTransportById(vehicle.id)).unwrap()
                    setDialogOpen(false)
                    setDialogConfig({
                        title: 'Success',
                    ...
                    setDialogConfig({
                        title: 'Error',
                        description: `Failed to delete vehicle: ${error}`,
                        actionText: 'OK',
                        color: 'red',
                        onConfirm: () => setDialogOpen(false),
                    })
    ...
    const handleFormSubmit = async (values: Record<string, any>) => {
        ...
        // The AddVehicleForm already handles the Redux dispatch
        // This callback is just for showing success dialog
        try {
            if (editingVehicle) {
                setDialogConfig({
                    title: 'Success',
                    description: `Vehicle "${values.vehicleType} - ${values.vehicleNumber}" updated successfully!`,
                    actionText: 'OK',
                    color: 'green',
                    onConfirm: () => setDialogOpen(false),
                })
                ...
                setDialogConfig({
                    title: 'Success',
                    description: `Vehicle "${values.vehicleType} - ${values.vehicleNumber}" added successfully!`,
                    actionText: 'OK',
                    color: 'green',
                    onConfirm: () => setDialogOpen(false),
                })
            ...
            setDialogConfig({
                title: 'Error',
                description: `Failed to ${editingVehicle ? 'update' : 'add'} vehicle: ${error}`,
                actionText: 'OK',
                color: 'red',
                onConfirm: () => setDialogOpen(false),
            })
